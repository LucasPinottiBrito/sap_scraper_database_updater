...
def get_note_details_and_attachments(ambiente_selecionado: str, regiao_selecionada: str, notes: list):
    logon = SapLogonScreen()
    login = logon.loadSystem(regiao_selecionada, ambiente_selecionado)
    home = login.login(usuario, senha, regiao_selecionada, ambiente_selecionado)
...
if __name__ == "__main__":
    ...
    sp_notes["region"] = "SP"
    get_note_details_and_attachments("EP1", "SP", sp_notes.to_dict(orient='records'))
    print("ES")
    es_notes = update_notes_from_environment("EP2", "ES")
    es_notes["region"] = "ES"
    get_note_details_and_attachments("EP2", "ES", es_notes.to_dict(orient='records'))
